...
def AI(board : chess.Board, depth, alpha, beta):
    ...
    if board.turn == True:
        ...
        best_legal_squares = []
        for square in y[0]:
            thing = board.piece_at(square.item())
            if(thing != None and thing.color == True):
                best_legal_squares.append(square)
                count += 1
            if count >= 5:
                break
        ...
        for move in board.legal_moves:
            if(move.from_square in best_legal_squares):
                board.push(move)
        ...
        for square in y[0]:
            thing = board.piece_at(square.item())
            if(thing != None and thing.color == False):
                best_legal_squares.append(square)
                count += 1
            if count >= 5:
                break
